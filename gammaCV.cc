...
int main(int argc, char* argv[]){
  ...
  for (int trackbarVal = 0; trackbarVal <= TRACKBAR_MAX ; ++trackbarVal)
  {
    ...
    {
      lookUpTable[trackbarVal][val] = (unsigned char)round(pow((double)val/(HISTOGRAM_SIZE-1), gamma)*(HISTOGRAM_SIZE - 1));
    }
  }
  namedWindow(windowName, CV_WINDOW_AUTOSIZE);
  const string trackbarName = "Gammma";
  createTrackbar(trackbarName, windowName, &trackbarGamma, TRACKBAR_MAX, changeGamma);
