...
void myintegral(Mat& org, Mat& sum);
...
int main(int argc, char* argv[]) {
  ...
  for (int ksize = KMIN; ksize <= KMAX; ksize+=2) {
    ...
    cout << "myBoxFilter: " << (getTickCount()-t)/tickFreqMS << "ms\t";

    t = getTickCount();                       // 開始時刻
    myBoxFilter(sourceImage, resultImage, Size(ksize, ksize));
                               // 自ら定義したintegral画像による box フィルタの適用
    cout << "myBoxFilter2: " << (getTickCount()-t)/tickFreqMS << "ms" << endl;
